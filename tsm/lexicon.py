...
from tsm.util import raw_graph_to_all_graphs, raw_pron_to_all_prons, process_pron
import zhon.hanzi
...
class Lexicon(dict):
    ...
    def write_to_lexicon(self, dest_path):
        out_lines = ["{} {}".format(e.grapheme, e.phonemes) for e in flatten(self.values())]
        write_lines_to_file(dest_path, out_lines)
    ...
    @classmethod
    def from_hakka_dict(cls,
                        dictionary_path,
                        grapheme_key: str = "對應國語",
                        phoneme_key: str = "四縣腔音讀",
                        delimiter: str = "\t"):
        df = pd.read_csv(dictionary_path, sep=delimiter, dtype={grapheme_key: str, phoneme_key: str})
        df = df.replace(np.nan, "", regex=True)
        all_entries = []
        all_raw_entries = list(zip(df[grapheme_key], df[phoneme_key]))
        for raw_graph, raw_pron in all_raw_entries:
            if raw_graph.strip() == "" or raw_pron.strip() == "":
                continue
            entries = list(product(raw_graph_to_all_graphs(raw_graph), [" ".join(re.findall("[a-z]+\d+", raw_pron))]))
            all_entries.append(entries)

        return cls([LexiconEntry(graph, 0.0, pron, Stratum.白) for graph, pron in chain(*all_entries)])
