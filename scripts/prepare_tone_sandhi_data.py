...
if __name__ == '__main__':
    ...
    parser.add_argument('--moses-bin-dir', required=True)
    ...
    if next(iter(pairs.values()))[1] is None:
        ...
        if args.src_to_phn_model_dir:
            ...
            subprocess.run([args.moses_path, '-threads', str(args.moses_threads), '-f', args.src_to_phn_model_dir], stdin=fi_src, stdout=fo_phn)
            fo_phn.close()
    ...
    fo_tgt = open(args.tgt_output_file)
    fo_ali = open(args.tgt_ali_file)
    with open(args.src_output_file) as fo_src:
        print("running moses")
        with subprocess.Popen([args.moses_path, '-threads', str(args.moses_threads), '-print-alignment-info', '-f', args.moses_model_path],
    ...
    tokenizer = BertNerTokenizer(args.minibert_tokenizer_path)
    ...
    for wavfile, parsed_sent in zip(pairs, parsed_sents):
        fo_tgt.write(re.sub(r'\s+', ' ', str(parsed_sent)) + "\n")
    fo_tgt_tree.close()
